...
typecheckInfixPats(Pat1,Pat2,HT,Env) :-
  typecheckPat(Pat1, HT, HEnv),
  typecheckPat(Pat2, _, TEnv),
  append(HEnv, TEnv, Env).
...
typecheckPat(infixpat(::,Pat1,Pat2), listOf(LT), Env) :- typecheckInfixPats(Pat1, Pat2, LT, Env), !.
...
typecheckDec(Env,bindvalrec(_,E),NewEnv) :- 
  append(Env, SEnv, NewEnv),
  typecheckExp(NewEnv, E, _),
  closeFunTypes(SEnv), !.
...
typecheckSequence(Env,[H],HT) :-
  typecheckExp(Env,H,HT),
  !.
typecheckSequence(Env, [H|T], HT) :-
  typecheckExp(Env, H, _),
  typecheckSequence(Env, T, HT),
  !.
...
%
%typecheckExp(Env, func(_,[H|T],_) :-
%        typecheckMatches([('handle@',fn(enx,_))| Env], 'handle@', T), append() !.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

typecheckExp(Env,func(_,ML),fn(Alpha,Beta)) :- 
      typecheckMatches([('anon@',fn(Alpha,Beta))|Env],'anon@',ML), !.
...
typecheckExp(Env, caseof(Exp1, ML), T) :-
  typecheckExp(Env, Exp1, T),
  typecheckMatches(Env, ML, _).

typecheckExp(Env, whiledo(Exp1,Exp2), T) :-
  typecheckExp(Env, Exp1, bool),
  typecheckExp(Env, Exp2, T).
