...
open MLAS;
	...
	and localBindings(ast,pats,globalBindings,scope) = 
		 let val freeVars = ref []
			...
			fun bindingsOf(int(n),bindings,scope) = ()
		     ...
		     | bindingsOf(whiledo(exp1,exp2),bindings,scope) = (
				 bindingsOf(exp1,bindings,scope);
				 bindingsOf(exp2,bindings,scope)
			)
	...
	| codegen(negate(t), outFile, indent, consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) = let
		val index = lookupIndex("-1",consts)
	in
		codegen(t, outFile, indent, consts,locals,freeVars,cellVars,globals,env,globalBindings,scope);
		TextIO.output(outFile, indent^"LOAD_CONST "^index^"\n");
		TextIO.output(outFile, indent^"BINARY_MULTIPLY\n")
	end

	| codegen(boolval(b),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) = let
		val value = if b = "true" then "True" else "False"
		val index = lookupIndex(value,consts)
	in
		TextIO.output(outFile,indent^"LOAD_CONST "^index^"\n")
	end

	| codegen(str(s),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) = let
		val index = lookupIndex(s,consts)
	in
		TextIO.output(outFile,indent^"LOAD_CONST "^index^"\n")
	end
	...
	| codegen(whiledo(t1,t2),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) = let
   		val startlabel = nextLabel()
   		val exitlabel = nextLabel()
   		val endlabel = nextLabel()
	in
		TextIO.output(outFile,indent ^ "SETUP_LOOP " ^ endlabel ^ "\n");
		TextIO.output(outFile, startlabel ^ ":\n");
		codegen(t1,outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope);
		TextIO.output(outFile, indent ^ "POP_JUMP_IF_FALSE " ^ exitlabel ^ "\n");
		codegen(t2,outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope);
		TextIO.output(outFile, indent ^ "JUMP_ABSOLUTE " ^ startlabel ^ "\n");
		TextIO.output(outFile, exitlabel ^ ":\n");
		TextIO.output(outFile, indent ^ "POP_BLOCK " ^ "\n");
		TextIO.output(outFile, endlabel ^ ":\n")
	end
	                        
	| codegen(apply(id("ref"),t2),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) =
      codegen(t2,outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope)
	| codegen(apply(id("!"),t2),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) =
		   codegen(t2,outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope)
	| codegen(infixexp(":=",id(name),t2),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) =
	  let val variable = codegen(t2,outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope)
		   val noneIndex =
		         lookupIndex("None",consts)
	  in
		 store(name,outFile,indent,locals,freeVars,cellVars,globals,env);
	...
	| codegen(infixexp("div",t1,t2),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) = let
		val _ = codegen(t1,outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope)
		val _ = codegen(t2,outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope)              
	in
	...
	| codegen(raisexp(t),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) = let
		val _ = codegen(t,outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope)      
	in
	...
	decgen(bindval(idpat(name),exp),outFile,indent,consts,locals,freeVars,cellVars,globals,env,globalBindings,scope) = 
	...
	nestedfuns(ast,outFile,indent,globals,env,globalBindings,scope) =
	 ...
	 fun compile filename  = 
		 let val (ast, _) = parse filename
			 ...
			 val consts = removeDups ("None"::"'Match Not Found'"::"0"::"-1"::(constants ast))
